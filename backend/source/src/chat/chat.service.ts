...
import { BanEnity, BlockPeople, Message, MuteEntity, User } from "src/typeorm";
...
export class ChatService {
	...
	isBlocked = (blocked: any[], block_me: any[], id: number) => {
		for (let i of blocked) {
			if ((parseInt(i.to_id.toString()) === id))
				return true
		}
		...
		try {
			...
			for (let i of rtn) {
				...
				try {
					...
					usr = username
					pp = pp
					if (!this.isBlocked(blocked, block_me, id))
